...
now = datetime.now()
DATE_STR = now.strftime("%Y%m%d")  # 20260521 格式，用于缓存
DISPLAY_DATE = now.strftime("%Y-%m-%d")  # 2026-05-21 格式，用于显示
...
def get_latest_cache():
    ...
    for f in os.listdir(CACHE_DIR):
        if f.endswith(".html") and f[:8].isdigit():
            files.append(f)
    if not files:
        return None
...
def save_cache(html):
    ...
    cache_file = os.path.join(CACHE_DIR, DATE_STR + ".html")
    with open(cache_file, "w", encoding="utf-8") as f:
        f.write(html)
    log("  已缓存: " + cache_file)

# ── 新闻功能 ──────────────────────────────────────
def fetch_concept_news(concept_name, max_news=2):
    """获取概念相关新闻（东方财富API）"""
    import urllib.parse
    try:
        # 搜索概念相关新闻
        kw = urllib.parse.quote(concept_name)
        url = f"http://newsapi.eastmoney.com/kuaixun/v2/api/list?q={kw}&lmit={max_news}"
        data = safe_json(url)
        if data and data.get("news"):
            news_list = []
            for n in data["news"][:max_news]:
                title = n.get("title", "")
                digest = n.get("digest", "") or n.get("simdigest", "")
                if title and digest:
                    # 截取摘要核心
                    digest = digest[:80] + "..." if len(digest) > 80 else digest
                    news_list.append({"title": title, "digest": digest})
            return news_list
    except Exception as e:
        log(f"    [新闻] {e}")
    return []
...
def gen_concept_card(c, rank):
    ...
    # 新闻区 - 调用真实新闻API，对TOP3概念获取新闻，其他用默认
    news_list = c.get("news", [])
    if not news_list and rank <= 3:
        # 对排名前3的概念实时获取新闻
        news_list = fetch_concept_news(name, 1)
        c["news"] = news_list
    
    if news_list:
        # 有真实新闻，显示第一条
        n = news_list[0]
        news_text = n.get("digest", "") or n.get("title", "")
    else:
        # 无新闻，显示默认
        news_text = name + "板块今日表现活跃"
    
    news_html = '<div class="concept-news"><div class="concept-news-text"><strong>' + esc(name) + '</strong>：' + esc(news_text) + '</div></div>'
...
def main():
    log("=" * 50)
    log("今天炒什么 | " + DISPLAY_DATE + " " + DOW_STR + " " + PERIOD)
    log("=" * 50)
